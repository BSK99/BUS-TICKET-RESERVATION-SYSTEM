/*This is the mini project based on the BUS RESERVATION SYSTEM */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
char ch[10][130]={"jarvis travels","kapoor Tours and Travels","AMC travels","pick and drop","GRC travels"};
char name[32][100]={'\0'};
char number[32][2]={'\0'};
int num1[32]={0};
int trno;
...
int main()
{
    printf("THIS PROGRAM IS THE MINI PROJECT BASED ON THE BUS RESERVATION SYSTEM ");
	//login();
    int num,i;
    do{
    //system("cls");
   ...
   printf("**************** WELCOME TO THE BUS RESERVATION SYSTEM ******************\n\n\n");
    ...
    printf("\n");
    printf("\t\t\t\t\t 3. cancellation of tickets\n");
    printf("\n");
    printf("\t\t\t\t\t 4. status of bus\n");
    printf("\n");
    printf("\t\t\t\t\t 5. Exit from reservation system\n\n");
...
void name_number(int booking,char numstr[100])
{
char tempstr[100],tempstr1[12]="status",tempstr2[12]="number", content1[100], content2[100], seatnumstr[100], inputstr[100], prevstr[100];
int number;
    FILE *a,*b;
    int i=0;
   strcat(numstr,".txt");
   ...
   strcat(tempstr2,numstr);
    a = fopen(tempstr1,"r+");
    fgets(content1,100,a);
    printf("File booking content 1 - %s",content1);
    fclose(a);

    b = fopen(tempstr2,"r+");
    fgets(content2,100,b);
    printf("File booking content 2 - %s",content2);
    fclose(b);
   a = fopen(tempstr1,"a");//for open the file to write the name in the file
   b = fopen(tempstr2,"a");
   memset(inputstr,0,strlen(inputstr));
    memset(prevstr,0,strlen(prevstr));
    memset(seatnumstr,0,strlen(seatnumstr));
for(i=0; i<booking; i++)//for entering the person name and seat number in the file
{
    printf("*******************Enter the details for ticket no %d*******************\n\n\n",i+1);
      ...
      scanf("%d",&number);
      strcpy(prevstr, seatnumstr);
      itoa(number,seatnumstr,100);
      strcat(inputstr, prevstr);
      printf("Input dtring is %s\n", inputstr);
      while(strstr(content2, seatnumstr) != NULL || strstr(inputstr, seatnumstr) != NULL) {
        printf("Seat number %d is already booked. Please choose another seat", number);
         printf("\t\t\t\tEnter the seat number:--->");
      scanf("%d",&number);
      itoa(number,seatnumstr,100);
      }
